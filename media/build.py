...
def build(slug, preview=None):
    ...
    default_trust = [
        ...
        ("断定しません", "「研究ではこうだった」と精度や限界を添えて紹介します。"),
    ]
    trust_html = ""
    if not mag:
        trust_items = cfg.get("trust") or default_trust
        trust_html = '<div class="trust">' + "".join(f'<div><b>{html.escape(t)}</b>{html.escape(d)}</div>' for t, d in trust_items) + '</div>'
    out = f'''<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8">
